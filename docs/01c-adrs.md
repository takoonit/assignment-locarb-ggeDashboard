...
# ADR-027: Use server-authorized admin data access for first load
**Status:** Proposed
**Date:** 2026-05-22
**Context:** The protected admin page is interactive and uses TanStack Query for CRUD tables, but rendering it as an empty client shell makes the first visible table wait for hydration and a browser-side API request. Admin reads also need one durable authorization boundary so server preloading, API routes, and future admin surfaces cannot drift in how they check access. Compile speed was also affected by forcing selected MUI packages through `transpilePackages`.
**Decision:** Admin read access should go through a server-only data access boundary that performs authorization, input normalization, and minimal DTO selection before returning paged table data. The `/admin` page should preload only the active admin tab through that boundary and hydrate the same React Query key used by the client. React Query remains the client-side interaction layer for tab changes, pagination, and mutations. `requireAdmin()` should be request-memoized so repeated checks in one render pass reuse the same session result. The explicit MUI `transpilePackages` override should stay removed while Next.js 16 and the current dependency versions build successfully without it.
**Consequence:** The active admin table can render from server-provided query data without an initial browser-only loading pass, while inactive tabs still fetch lazily when selected. A shared server-only admin data boundary reduces authorization drift between page preloading and API routes, at the cost of a small additional abstraction that future admin reads must use. Removing `transpilePackages` reduces unnecessary compile work, but should be rechecked after MUI or Next.js upgrades.
