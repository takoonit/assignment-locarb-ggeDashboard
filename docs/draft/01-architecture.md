...
flowchart TB
    subgraph Browser
        UI[Dashboard UI<br/>React 19.2 + shadcn/ui]
...
Within the Next.js app, code is organized in concentric layers:
...
sequenceDiagram
    ...
    alt Cache hit (fresh)
        ...
        alt Invalid params
            ...
            S->>D: SELECT year, total_emissions WHERE country_code = ?
            D-->>S: Rows
...
sequenceDiagram
    ...
    P->>P: Read session cookie
    alt No session
        P-->>B: 401 Unauthorized
    else Has session
        P->>R: Forward with session
        R->>A: requireAdmin(session)
        alt Not admin
            A-->>B: 403 Forbidden
        else Admin
            R->>D: DELETE FROM emission WHERE id = 123
            D-->>R: { count: 1 }
            R-->>B: 204 No Content
...
**Why this matters:** the two flows show that authorization happens at one chokepoint (`requireAdmin` helper), not scattered across handlers. The proxy only checks *presence* of session; route handlers check *role*.
...
2. Actions runs `lint`, `test`, `build` against a Neon test branch
